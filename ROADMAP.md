...
| F-5 ⏸ | **`sqlite-vec` ANN vector index** (also E-10) — drop-in for the existing swappable vector seam; removes the brute-force ceiling. **Deferred (decision 2026-07): native sqlite-vec is a dlopen'd SQLite extension, which `bun build --compile` cannot embed** — the same wall imagescript's native codec hit (`scripts/patch-imagescript.ts` stubs it, falls back to WASM). It could only ship as an *opt-in* (env → extension path, off in the default binary), which is a poor fit for a single-binary product's marquee feature, and there's no concrete large-collection user yet. The seam (`vectorSearch` in `core/records.ts`, `topK` in `vector.ts`) is already isolated, so this stays a clean drop-in whenever it's revisited. **Two viable paths when it is:** (a) opt-in sqlite-vec + a `vec0` index with write-sync; (b) a pure-JS/WASM in-process ANN (e.g. HNSW) that survives the single binary. Today's brute-force scan keeps its cap + `truncated` transparency. | `core/records.ts` vectorSearch, `core/vector.ts:128` | M |
...
| F-15 ✅ | **Realtime presence** (Supabase-style) — ephemeral "who's on a channel + their state" over WS (`presence-track`/`-untrack`/`-state` messages) with join/update/leave deltas + a full snapshot; a read-only HTTP snapshot (`GET /api/v1/realtime/presence/:channel`) for SSE observers. Backed by `cogworks_presence` so it's cross-worker-correct with no extra plumbing — changes fan out via the existing `broadcastSystem` topic path; a per-worker heartbeat + reaper cull a crashed worker's rows. `identity` is server-attached from auth (unspoofable); `state`/`key` are the client's. Was previously "skip — no pull yet"; built on request. | `realtime/presence.ts`, `server.ts` | M |
...
- **KMS/HSM key storage** — defer; folds into **F-13** later. **tus resumable uploads / ClamAV scanning** — defer (**E-5** covers streaming). **Extra-language SDKs (Py/Go/Swift) + plugin system** — defer until **F-3** proves the generator. ~~**Realtime presence/broadcast channels** — no strong pull yet.~~ → **shipped as F-15** (presence). Broadcast-only channels (no roster) remain skippable — presence already carries arbitrary `state`.
